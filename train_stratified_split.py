...
for c in classes:
    ...
    for img in train_imgs:
        shutil.copy(os.path.join(class_path, img), os.path.join(output_dir, "train", c, img))
    for img in val_imgs:
        shutil.copy(os.path.join(class_path, img), os.path.join(output_dir, "val", c, img))
    for img in test_imgs:
        shutil.copy(os.path.join(class_path, img), os.path.join(output_dir, "test", c, img))
...
val_gen = eval_datagen.flow_from_directory(                                # CHANGED #4: new val generator
...
# CHANGED #5: validation_data=val_gen (NOT test_gen)
history1 = model.fit(train_gen, epochs=25, validation_data=val_gen, callbacks=[early_stop],
...
early_stop_fine = EarlyStopping(monitor='val_loss', patience=7, restore_best_weights=True)
history2 = model.fit(train_gen, epochs=30, validation_data=val_gen, callbacks=[early_stop_fine],
                     class_weight=class_weight_dict)
...
plt.title('Stratified Directory Split: Accuracy Curve')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.grid(True)
...
plt.axvline(x=phase1_epochs - 1, color='red', linestyle='--', label='Phase 2 Transition')
plt.title('Stratified Directory Split: Loss Curve')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
plt.tight_layout()
plt.savefig('stratified_split_learning_curves.png', dpi=300)
print("\n[INFO] Learning curves saved as 'stratified_curves.png'")
plt.show()

print("\n--- Evaluating Model on Held-Out Test Set ---")
loss_eval, accuracy_eval = model.evaluate(test_gen)
...
print(f"Stratified Directory Split Test Accuracy : {accuracy_eval * 100:.2f}%")
